...
describe("detectAmountOutlier", () => {
  ...
  it("fires when a spike sits outside the band and beyond 2σ", () => {
    ...
    expect(result!.bandNearEdgeCents).toBe(BigInt(-518_660));
    expect(result!.bandFarEdgeCents).toBe(BigInt(-667_775));
  ...
  it("fires a 7% hike on a previously flat variable bill (σ = 0)", () => {
    ...
    expect(result!.bandNearEdgeCents).toBe(BigInt(-42_000));
    expect(result!.bandFarEdgeCents).toBe(BigInt(-42_000));
  });
